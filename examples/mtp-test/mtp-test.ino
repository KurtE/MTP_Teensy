...
#define USE_LFS_QSPI_FLASH 0 // T4.1 QSPI
...
SDMTPClass mySD[] = {{mtpd, storage, "SDIO", CS_SD},
...
#if USE_LFS_RAM == 1 || USE_LFS_PROGM == 1 || USE_LFS_QSPI == 1 ||             \
    USE_LFS_QSPI_FLASH == 1 || USE_LFS_SPI == 1 || USE_LFS_NAND == 1 ||        \
    USE_LFS_QSPI_NAND == 1
//=============================================================================
// LittleFS classes
//=============================================================================
...
// =======================================================================
#if USE_LFS_RAM == 1
const char *lfs_ram_str[] = {"RAM1",
                             "RAM2"}; // edit to reflect your configuration
const int lfs_ram_size[] = {200'000,
                            4'000'000}; // edit to reflect your configuration
...
LittleFS_QSPI qspifs[nfs_qspi];
#endif

#if USE_LFS_QSPI_FLASH == 1
const char *lfs_qspi_str_flash[] = {
    "QSPIF"}; // edit to reflect your configuration
const int nfs_qspi_flash = sizeof(lfs_qspi_str_flash) / sizeof(const char *);

LittleFS_QSPIFlash qspifs_flash[nfs_qspi_flash];
#endif
...
#endif
#if USE_LFS_NAND == 1
const char *nspi_str[] = {"WINBOND1G",
                          "WINBOND2G"}; // edit to reflect your configuration
const int nspi_cs[] = {3, 4};           // edit to reflect your configuration
const int nspi_nsd = sizeof(nspi_cs) / sizeof(int);
LittleFS_SPINAND
    nspifs[nspi_nsd]; // needs to be declared if LittleFS is used in storage.h
#endif
...
void storage_configure() {
  ...
  for (int ii = 0; ii < nfs_ram; ii++) {
    if (!ramfs[ii].begin(lfs_ram_size[ii])) {
      DBGSerial.printf("Ram Storage %d %s failed or missing", ii,
                       lfs_ram_str[ii]);
      DBGSerial.println();
    } else {
      storage.addFilesystem(ramfs[ii], lfs_ram_str[ii], &lfsmtpcb,
                            (uint32_t)(LittleFS *)&ramfs[ii]);
      uint64_t totalSize = ramfs[ii].totalSize();
      uint64_t usedSize = ramfs[ii].usedSize();
      DBGSerial.printf("RAM Storage %d %s ", ii, lfs_ram_str[ii]);
  ...
  for (int ii = 0; ii < nfs_progm; ii++) {
    if (!progmfs[ii].begin(lfs_progm_size[ii])) {
      DBGSerial.printf("Program Storage %d %s failed or missing", ii,
                       lfs_progm_str[ii]);
      DBGSerial.println();
    } else {
      storage.addFilesystem(progmfs[ii], lfs_progm_str[ii], &lfsmtpcb,
                            (uint32_t)(LittleFS *)&progmfs[ii]);
  ...
  for (int ii = 0; ii < nfs_qspi; ii++) {
    if (!qspifs[ii].begin()) {
      DBGSerial.printf("QSPI Storage %d %s failed or missing", ii,
                       lfs_qspi_str[ii]);
      DBGSerial.println();
    } else {
      storage.addFilesystem(qspifs[ii], lfs_qspi_str[ii], &lfsmtpcb,
                            (uint32_t)(LittleFS *)&qspifs[ii]);
...
#if USE_LFS_QSPI_FLASH == 1
  for (int ii = 0; ii < nfs_qspi_flash; ii++) {
    if (!qspifs_flash[ii].begin()) {
      DBGSerial.printf("QSPI Storage %d %s failed or missing", ii,
                       lfs_qspi_str_flash[ii]);
      DBGSerial.println();
    } else {
      storage.addFilesystem(qspifs_flash[ii], lfs_qspi_str_flash[ii], &lfsmtpcb,
                            (uint32_t)(LittleFS *)&qspifs_flash[ii]);
      uint64_t totalSize = qspifs_flash[ii].totalSize();
      uint64_t usedSize = qspifs_flash[ii].usedSize();
      DBGSerial.printf("QSPI Storage %d %s ", ii, lfs_qspi_str_flash[ii]);
      DBGSerial.print(totalSize);
      DBGSerial.print(" ");
      DBGSerial.println(usedSize);
    }
  ...
  for (int ii = 0; ii < nfs_spi; ii++) {
    ...
    if (!spifs[ii].begin(lfs_cs[ii], SPI)) {
      DBGSerial.printf("SPIFlash Storage %d %d %s failed or missing", ii,
      ...
      DBGSerial.printf("SPIFlash Storage %d %d %s ", ii, lfs_cs[ii],
                       lfs_spi_str[ii]);
      DBGSerial.print(totalSize);
      DBGSerial.print(" ");
      DBGSerial.println(usedSize);
  ...
  for (int ii = 0; ii < nspi_nsd; ii++) {
    ...
    if (!nspifs[ii].begin(nspi_cs[ii], SPI)) {
      DBGSerial.printf("SPIFlash NAND Storage %d %d %s failed or missing", ii,
                       nspi_cs[ii], nspi_str[ii]);
      DBGSerial.println();
    } else {
      storage.addFilesystem(nspifs[ii], nspi_str[ii], &lfsmtpcb,
                            (uint32_t)(LittleFS *)&nspifs[ii]);
  ...
  for (int ii = 0; ii < qnspi_nsd; ii++) {
    if (!qnspifs[ii].begin()) {
      DBGSerial.printf("QSPI NAND Storage %d %s failed or missing", ii,
                       qnspi_str[ii]);
      DBGSerial.println();
    } else {
      storage.addFilesystem(qnspifs[ii], qnspi_str[ii], &lfsmtpcb,
                            (uint32_t)(LittleFS *)&qnspi_str[ii]);

      uint64_t totalSize = qnspifs[ii].totalSize();
      uint64_t usedSize = qnspifs[ii].usedSize();
      DBGSerial.printf("Storage %d %s ", ii, qnspi_str[ii]);
      DBGSerial.print(totalSize);
      DBGSerial.print(" ");
      DBGSerial.println(usedSize);
    }
